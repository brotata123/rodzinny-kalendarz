...
function _renderOneLessonCard(l, idx, editMode) {
  if (l.spacer) {
    const delBtn = editMode
      ? `<button class="plan-delete-card-btn" onclick="planDeleteLesson(${idx})">×</button>`
      : '';
    return `<div class="plan-spacer${editMode ? ' editable' : ''}">${delBtn}</div>`;
  }
  ...
  const editAttrs = editMode
    ? `class="plan-lesson-card editable" onclick="openLessonEditor(${idx})"`
    : 'class="plan-lesson-card"';
  const delBtn = editMode
    ? `<button class="plan-delete-card-btn" onclick="event.stopPropagation();planDeleteLesson(${idx})">×</button>`
    : '';
...
async function resetPlanDay() {
  ...
  try {
    await db.collection('families').doc(familyId).collection('plan_overrides')
      .doc(String(currentPlanDay)).delete();
